...
func (c Clusters) Neighbour(point Coordinates, fromCluster int) (int, float64) {
	var d float64
	nc := -1
...
// Set all cluster centers
func (c Clusters) SetCc(newCenters []Coordinates) {
	lenC := len(c)
	for i := 0; i < lenC; i++ {
		c[i].Center = newCenters[i]
